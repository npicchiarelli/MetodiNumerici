...
for maxeta in etamax
    ...
    fitx2 = curve_fit(model, esq[msk], x2, x2v.^(-2), [0., 0.])
    println("x2")
    println(fitx2.param)
    ...
    fitH = curve_fit(model, esq[msk], H, Hv.^(-2), [0., 0.])
    println("H")
    println(fitH.param)
...
plot!(fitotx2, [0, 0.25], [cst, cst], lw=1, lc=:black, ls =:dash, label = "Analytical")
title!(fitotx2, L"$\langle x^2 \rangle$ extracted by fit")
...
plot!(fitotH, [0, 0.25], [cst, cst], lw=1, lc=:black, ls =:dash, label = "Analytical")
title!(fitotH, L"$\langle H \rangle$ extracted by fit")
